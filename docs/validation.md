...
## Ranking synonym and snapshot scope fixes verified September 15, 2026

“Biggest building gaps” now requests descending building-gap ranking in guided and local-model modes. Explicit counts, group filters, and top-tract inspection requirements are retained. Any explicit 11-digit ID absent from the loaded snapshot declines the whole request before analytical tools or inference, with an Out of scope explanation and a suggestion to choose a covered tract. This applies to IDs outside California and absent IDs with a California prefix; absence does not establish that a tract is geographically outside Northern California. Model-generated bad IDs still receive tool feedback and can be corrected.

All 34 tests and 11 controlled evaluations passed. Both requested cases passed in the refreshed local-model batch: biggest building gaps completed, and the outside-snapshot request was declined before inference. Overall 5/6 local request expectations passed; the separate ranking-plus-inspection case remained Partial after two rejected calls and one completion reminder. The comparison replay and source provenance were refreshed. The local HTTP endpoint also returned complete building ranking and an explicit scope response with no tool calls for an unavailable ID. Type checking and lint passed.
