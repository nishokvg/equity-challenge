...
Model traces retain the exact raw argument string, including malformed JSON. The interface and exported report preserve it beside any corrected call. Rejected calls consume the same model-selected tool budget. Formula consistency validates arithmetic against shared aggregated counts; it does not validate spatial joins, clipping, or source feature counting. The dedicated missing-reference check includes both halves of places as well as roads and buildings.

Geist Sans and Geist Mono are bundled in `public/fonts`, with the upstream SIL Open Font License and pinned source hashes. Neither building nor displaying fonts requires Google Fonts. Install dependencies beforehand; `npm run build` then uses local font assets.

Ranking accepts “biggest” (for example, “Biggest building gaps” or “Biggest 3 rural building gaps”). Explicit tract IDs are checked against the loaded snapshot before tools or inference. A request containing any unavailable ID returns an out-of-scope explanation and asks the user to remove that ID or select a covered tract; it does not silently analyze only the supported portion. Invalid IDs generated by the model during a supported request still receive tool feedback and can be corrected.
